 ...
 * Note: since we already have the ManyToOne relationship between warehouse & product, by definining
 * the productId & warehouseId (same name as the one created in the relationship) as PKs, we are basically
 * creating a Constraint with those properties - meaning that we can have WarehouseStock records without both.
 */
@Entity()
